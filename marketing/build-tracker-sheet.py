...
info_cols = ["Priority", "Sport", "Segment", "Creator", "Platform & Handle", "Followers", "Style", "Why fit"]
track_cols = ["Ref code", "Outreach date", "Channel", "Rate (their quote)", "Replied?", "Posted?", "Post type", "New players", "Paid?", "Notes"]
cols = info_cols + track_cols
widths = [9, 11, 22, 24, 30, 13, 26, 34,  10, 13, 10, 14, 9, 9, 12, 12, 8, 26]
...
for i, r in enumerate(rows):
    ...
    for j, v in enumerate(vals, 1):
        cell = ws.cell(rr, j, v)
        cell.border = border
        cell.alignment = wrap if j in (3,4,5,7,8,18) else top
        if j <= len(info_cols):
            pk = pkey(r["Priority"])
...
dv("K", '"X DM,Email,IG DM,TikTok DM,Other"')       # Channel
...
sheet_block(pay, "Pay & Ground Rules", [
    ...
    ("b", "$15 for every 1,000 new players your creators send us, each person counted once (not raw plays), tracked through the links (mostly YouTube, X, Twitch)."),
...
sheet_block(tpl, "Outreach Templates (personalize the [brackets])", [
    ...
    ("b", "Fee: their quoted rate (they name it; [owner] approves before you commit), paid [50% upfront / on posting]"),
...
sheet_block(how, "How to use this sheet", [
    ...
    ("b", "New players = unique people from the tracking-link report, each counted once no matter how many times they play (ask [owner] for it); drives the $15/1,000 bonus."),
